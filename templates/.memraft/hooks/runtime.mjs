...
const SHARED_SPEC_SECTION_HEADING = "Accepted Entries";
const SHARED_SPEC_SECTIONS = {
  background: {
    label: "Background",
    relativePath: path.join("spec", "background.md"),
    title: "Shared Background",
  },
  conventions: {
    label: "Conventions",
    relativePath: path.join("spec", "conventions.md"),
    title: "Shared Conventions",
  },
  workflows: {
    label: "Workflows",
    relativePath: path.join("spec", "workflows.md"),
...
function getSharedSpecPaths(repoRoot) {
  const root = path.join(repoRoot, SHARED_SPEC_ROOT);
  return {
    root,
    registry: path.join(root, "registry.json"),
    sections: Object.fromEntries(
      Object.entries(SHARED_SPEC_SECTIONS).map(([sectionName, meta]) => [
        sectionName,
        {
          ...meta,
          path: path.join(root, meta.relativePath),
        },
      ]),
    ),
  };
...
function loadSharedSpecRegistry(repoRoot) {
  const paths = getSharedSpecPaths(repoRoot);
  const registry = readJson(paths.registry);
  if (registry && typeof registry === "object" && !Array.isArray(registry)) {
    return {
      version: Number.isInteger(registry.version) ? registry.version : 1,
      updatedAt: getStringValue(registry, "updatedAt"),
      entries:
...
function saveSharedSpecRegistry(repoRoot, registry) {
  const paths = getSharedSpecPaths(repoRoot);
...
function listSharedSpecEntries(registry) {
  const entries =
    registry?.entries && typeof registry.entries === "object" && !Array.isArray(registry.entries)
  ...
  return entries
    ...
    .sort((left, right) =>
      getStringValue(right, "acceptedAt").localeCompare(getStringValue(left, "acceptedAt")) ||
      getStringValue(left, "fingerprint").localeCompare(getStringValue(right, "fingerprint")),
    );
}

function loadSharedSpecSources(repoRoot) {
  const paths = getSharedSpecPaths(repoRoot);
  const registry = loadSharedSpecRegistry(repoRoot);
  const sections = Object.fromEntries(
    Object.entries(paths.sections).map(([sectionName, meta]) => [
      sectionName,
      {
        ...meta,
        acceptedText: readMarkdownSection(meta.path, SHARED_SPEC_SECTION_HEADING),
      },
    ]),
  );
  return {
    ...paths,
    registry,
    entries: listSharedSpecEntries(registry),
    sections,
  };
}

function renderSharedSpecLines(sharedSpecSources, {
  includeSectionHeadings = true,
  maxLinesPerSection = 8,
  maxCharsPerSection = 900,
} = {}) {
  const lines = [];
  for (const [sectionName, meta] of Object.entries(sharedSpecSources.sections ?? {})) {
    const prepared = prepareInjectedSection(meta.acceptedText ?? "", {
...
function buildSharedSpecSummary(sharedSpecSources) {
  const acceptedBySection = {};
  for (const sectionName of Object.keys(SHARED_SPEC_SECTIONS)) {
    acceptedBySection[sectionName] = 0;
  }
  for (const entry of sharedSpecSources.entries ?? []) {
    const acceptedInto = getStringValue(entry, "acceptedInto");
    if (acceptedInto && Object.hasOwn(acceptedBySection, acceptedInto)) {
      acceptedBySection[acceptedInto] += 1;
    }
  }
  return {
    root: sharedSpecSources.root,
    registryPath: sharedSpecSources.registry,
    acceptedEntries: (sharedSpecSources.entries ?? []).length,
    acceptedBySection,
  };
}
...
function getAssistantExcerpt(inputData, maxChars = 0) {
  const lastMessage = getLastAssistantMessage(inputData).trim();
  if (!lastMessage) {
    return { excerpt: "", chars: 0 };
  }
  if (!Number.isInteger(maxChars) || maxChars <= 0 || lastMessage.length <= maxChars) {
    return {
      excerpt: lastMessage,
      chars: lastMessage.length,
    };
  }
  return {
    excerpt: lastMessage.slice(0, maxChars),
    chars: lastMessage.length,
  };
}
...
function getStopSummaryConfig(config) {
  ...
  if (typeof stopSummary.mode === "string") {
    const normalizedMode = stopSummary.mode.trim().toLowerCase();
    if (["light", "full"].includes(normalizedMode)) {
      nextConfig.mode = normalizedMode;
    }
  }
  for (const key of [
    "minimumConversationChars",
...
function getCompileOutputPaths(repoRoot, config) {
  return {
    ...
    codexHooks: resolveMemraftPath(repoRoot, "generated/adapters/codex/hooks.json", {
      label: "generated.adapters.codexHooks",
    }),
...
function buildLightSessionEndPayload(event, files) {
  const assistantExcerpt = getStringValue(event, "assistantMessageExcerpt");
  return {
    summary: summarizeDeferredContext(assistantExcerpt, "", files),
    knowledge: extractDeferredKnowledge([assistantExcerpt]),
    candidate_spec: extractDeferredCandidateSpec([assistantExcerpt]),
  };
}
...
function connect(repoRoot) {
  ...
  try {
    ...
    const currentMode = typeof row?.journal_mode === "string" ? row.journal_mode.toLowerCase() : "";
    if (currentMode !== "wal") {
...
function buildAdapterModesFromStates(states) {
  ...
  return {
    codex: buildMode(
      isEnabled("nativeAgents"),
      isEnabled("nativeCodexConfig") && isEnabled("nativeCodexHooks"),
    ),
    opencode: buildMode(
      isEnabled("nativeAgents") && isEnabled("nativeOpencodeConfig"),
      isEnabled("nativeOpencodeConfig") && isEnabled("nativeOpencodePlugin"),
    ),
    gemini: buildMode(isEnabled("nativeGemini"), false),
  };
}
...
function inferSharedSpecSection(record, collectionName) {
  const kind = getStringValue(record, "kind").toLowerCase();
  const text = getStringValue(record, "text").toLowerCase();
  if (
    text.startsWith("keep ") ||
    text.startsWith("prefer ") ||
    text.startsWith("use ") ||
    text.startsWith("avoid ")
  ) {
    return "conventions";
  }
  if (
  ...
  if (collectionName === "knowledge" && (kind.endsWith("fact") || kind === "repo-fact")) {
    return "background";
  }
  return "conventions";
}

function listSharedSpecProposals(repoRoot) {
  ensureStorage(repoRoot);
  const registry = loadSharedSpecRegistry(repoRoot);
  const acceptedFingerprints = new Set(
    listSharedSpecEntries(registry)
      .map((entry) => getStringValue(entry, "fingerprint"))
      .filter(Boolean),
  );
  const db = connect(repoRoot);
  try {
    const rows = db
      .prepare(`
        SELECT memory_id, collection_name, record_json, updated_at
        FROM memories
        WHERE collection_name IN ('knowledge', 'candidateSpec')
    ...
    return rows
      ...
      .filter((item) => getStringValue(item.record, "lifecycleStatus") !== "invalidated")
      .filter((item) => !acceptedFingerprints.has(getStringValue(item.record, "fingerprint")))
      .map((item) => ({
        ...item,
        fingerprint: getStringValue(item.record, "fingerprint"),
        recommendedSection: inferSharedSpecSection(item.record, item.collection),
      }))
      .filter((item) => getStringValue(item.record, "sharedStatus") !== "accepted");
  } finally {
    db.close();
  }
}

function appendMarkdownBulletToSection(filePath, { title, heading, bullet }) {
  ...
  let headingIndex = lines.findIndex((line) => line.trim() === headingLine);
  if (headingIndex === -1) {
    while (lines.length > 0 && lines[lines.length - 1].trim() === "") {
      lines.pop();
    }
    lines.push("", headingLine, "", `- ${trimmedBullet}`, "");
  ...
  const bulletLine = `- ${trimmedBullet}`;
  if (lines.slice(headingIndex + 1, sectionEnd).some((line) => line.trim() === bulletLine)) {
    return false;
  }
...
function resolveSharedProposal(repoRoot, identifier) {
  const trimmedIdentifier = String(identifier ?? "").trim();
  if (!trimmedIdentifier) {
    throw new Error("accept requires a fingerprint");
  }
  const proposals = listSharedSpecProposals(repoRoot);
  const matches = proposals.filter(
    (item) =>
      item.memoryId === trimmedIdentifier ||
      item.fingerprint === trimmedIdentifier ||
      item.fingerprint.startsWith(trimmedIdentifier),
  );
  if (matches.length === 0) {
    throw new Error(`No promoted shared-spec proposal found for: ${trimmedIdentifier}`);
  }
  if (matches.length > 1) {
    throw new Error(`Proposal fingerprint prefix is ambiguous: ${trimmedIdentifier}`);
  }
  return matches[0];
}

function acceptSharedProposal(repoRoot, identifier, into, acceptedAt) {
  const sectionName = String(into ?? "").trim();
  if (!Object.hasOwn(SHARED_SPEC_SECTIONS, sectionName)) {
    throw new Error(`Unsupported shared spec section: ${sectionName}`);
  }
  const proposal = resolveSharedProposal(repoRoot, identifier);
  const sharedSpecPaths = getSharedSpecPaths(repoRoot);
  const sectionMeta = sharedSpecPaths.sections[sectionName];
  const registry = loadSharedSpecRegistry(repoRoot);
  const existingEntry = listSharedSpecEntries(registry).find(
    (entry) => getStringValue(entry, "fingerprint") === proposal.fingerprint,
  );
  if (existingEntry) {
    const acceptedInto = getStringValue(existingEntry, "acceptedInto");
    if (acceptedInto && acceptedInto !== sectionName) {
      throw new Error(
        `Proposal ${proposal.fingerprint} is already accepted into ${acceptedInto}.`,
      );
    }
  }
  appendMarkdownBulletToSection(sectionMeta.path, {
    title: sectionMeta.title,
    heading: SHARED_SPEC_SECTION_HEADING,
    bullet: getStringValue(proposal.record, "text"),
  });
  const entryId = `${sectionName}:${proposal.fingerprint}`;
  registry.updatedAt = acceptedAt;
  registry.entries = {
    ...(registry.entries ?? {}),
    [entryId]: {
      entryId,
      ...
      acceptedInto: sectionName,
      acceptedAt,
      text: getStringValue(proposal.record, "text"),
      kind: getStringValue(proposal.record, "kind"),
      scope: getStringValue(proposal.record, "scope"),
      sourceEvidenceIds: Array.isArray(proposal.record.sourceEvidenceIds)
        ? proposal.record.sourceEvidenceIds
        : [],
    },
  };
  saveSharedSpecRegistry(repoRoot, registry);
  ...
  try {
    ...
    db.prepare(`
      ...
      SET updated_at = ?,
          record_json = ?
      WHERE memory_id = ?
    `).run(acceptedAt, jsonDumps(acceptedRecord), proposal.memoryId);
  } finally {
    db.close();
  }

  writeAuditEntry(repoRoot, acceptedAt, "shared-spec.accepted", "memory", proposal.memoryId, {
    fingerprint: proposal.fingerprint,
    section: sectionName,
    entryId,
  });
...
function buildRuntimeSummary(repoRoot) {
  ensureStorage(repoRoot);
  const sharedSpecSources = loadSharedSpecSources(repoRoot);
  const db = connect(repoRoot);
  try {
    ...
    return {
      ...
      sharedSpec: buildSharedSpecSummary(sharedSpecSources),
...
function renderCompiledSpecMarkdown(repoProfile, knowledgeRecords, specRecords, sharedSpecSources) {
  const lines = [
  ...
  const sharedSpecLines = renderSharedSpecLines(sharedSpecSources, {
    includeSectionHeadings: true,
    maxLinesPerSection: 24,
    maxCharsPerSection: 2400,
  });
  if (sharedSpecLines.length > 0) {
    lines.push("## Shared Project Spec", "", ...sharedSpecLines, "");
  }
  lines.push(
...
function renderSessionStartInjection(repoRoot, repoProfile, latestEvidence, knowledgeRecords, specRecords, adapterModes, sharedSpecSources) {
  const lines = [
    "<memraft-context>",
...
function renderSharedInjection(repoRoot, repoProfile, latestEvidence, knowledgeRecords, specRecords, adapterModes, sharedSpecSources) {
  const lines = [
...
function renderAdapterMarkdown(toolLabel, { repoRoot, filename, repoProfile, latestEvidence, knowledgeRecords, specRecords, adapterModes, adapterName, sharedSpecSources }) {
  const lines = [
...
function createCodexHook(command, timeout, statusMessage = "") {
  const hook = {
    type: "command",
    command,
    timeout,
...
function isManagedCodexHookCommand(command, scriptName) {
  return typeof command === "string" && command.includes(`${scriptName}.mjs`);
}

function ensureCodexHookMatcher(config, eventName, matcher, hook, scriptName) {
  ...
  const existingHooks = Array.isArray(existingMatcher.hooks) ? existingMatcher.hooks : [];
  const nextHooks = [];
  let replaced = false;
  for (const entry of existingHooks) {
    ...
    const sameManagedHook =
      entry.type === hook.type && isManagedCodexHookCommand(entry.command, scriptName);
    const sameExactHook =
      entry.type === hook.type && entry.command === hook.command;
...
function buildCodexHooksConfig() {
  const config = { hooks: {} };
  ensureCodexHookMatcher(
    config,
    "SessionStart",
    "startup|resume",
    createCodexHook(
      `${getNodeCommand()} --input-type=module -e ${quoteShellArgument(buildRepoHookLoaderCode("session_start"))}`,
      10,
      "Loading Memraft session context",
    ),
    "session_start",
  );
  ensureCodexHookMatcher(
    ...
    undefined,
    createCodexHook(
      `${getNodeCommand()} --input-type=module -e ${quoteShellArgument(buildRepoHookLoaderCode("codex_user_prompt_submit"))}`,
      10,
      "Injecting Memraft task context",
    ),
  ...
  ensureCodexHookMatcher(
    ...
    undefined,
    createCodexHook(
      `${getNodeCommand()} --input-type=module -e ${quoteShellArgument(buildRepoHookLoaderCode("codex_stop"))}`,
      20,
      "Capturing Memraft evidence",
    ),
    "codex_stop",
  );
  return config;
...
function renderCodexHooks() {
  return `${JSON.stringify(buildCodexHooksConfig(), null, 2)}\n`;
}

function loadCodexHooksConfig(filePath) {
  const content = readText(filePath, "").trim();
  if (!content) {
    return { hooks: {} };
  }
...
function upsertCodexHooks(filePath) {
  const existing = loadCodexHooksConfig(filePath);
  if (existing === null) {
    return false;
  }
  const nextConfig = {
  ...
  for (const [eventName, entries] of Object.entries(buildCodexHooksConfig().hooks)) {
    for (const entry of entries) {
      const hook = Array.isArray(entry.hooks) ? entry.hooks[0] : null;
      if (!hook || typeof hook !== "object") {
        continue;
      }
      const scriptName = String(hook.command.match(/([a-z_]+)\.mjs/)?.[1] ?? "");
      ensureCodexHookMatcher(nextConfig, eventName, entry.matcher, hook, scriptName);
    }
  }
  const rendered = `${JSON.stringify(nextConfig, null, 2)}\n`;
  if (rendered !== readText(filePath, "")) {
    writeText(filePath, rendered);
  }
  return true;
}
...
function renderCodexConfig() {
  return [
    ...
    "# MEMRAFT:BEGIN codex-hooks",
    "features.codex_hooks = true",
    "# MEMRAFT:END codex-hooks",
    "",
  ].join("\n");
}
...
function upsertCodexConfig(filePath, managedContent) {
  const existing = readText(filePath, "");
  const blockPattern = /# MEMRAFT:BEGIN codex-hooks\r?\n[\s\S]*?\r?\n# MEMRAFT:END codex-hooks\r?\n?/m;
  let nextContent;
  if (blockPattern.test(existing)) {
    nextContent = existing.replace(blockPattern, managedContent);
  } else {
    const existingFlagMatch = existing.match(
      /^[ \t]*(?:features\.)?codex_hooks[ \t]*=[ \t]*(true|false)\b/m,
    );
    if (existingFlagMatch) {
      return existingFlagMatch[1] === "true";
    }
...
function canManageCodexConfig(filePath) {
  const existing = readText(filePath, "");
  const blockPattern = /# MEMRAFT:BEGIN codex-hooks\r?\n[\s\S]*?\r?\n# MEMRAFT:END codex-hooks/m;
  if (blockPattern.test(existing)) {
    return true;
  }
  const existingFlagMatch = existing
    .replace(blockPattern, "")
    .match(/^[ \t]*(?:features\.)?codex_hooks[ \t]*=[ \t]*(true|false)\b/m);
  return !existingFlagMatch || existingFlagMatch[1] === "true";
}

function canManageCodexHooks(filePath) {
  return loadCodexHooksConfig(filePath) !== null;
}
...
function planNativeAdapterStates(repoRoot, outputPaths) {
  ...
  return {
    ...
    nativeCodexConfig: [
      ...
      {
        path: entrypoints.codexConfig,
        mode: "feature-flag",
        enabled: canManageCodexConfig(entrypoints.codexConfig),
      },
    ],
    nativeCodexHooks: [
      canManageCodexHooks(entrypoints.codexHooks) ? "managed" : "conflict",
      {
        path: entrypoints.codexHooks,
        mode: "hooks-json-merge",
        enabled: canManageCodexHooks(entrypoints.codexHooks),
      },
    ],
...
function syncNativeAdapters(repoRoot, config = loadConfig(repoRoot), outputPaths = getCompileOutputPaths(repoRoot, config)) {
  ...
  const codexHooks = readText(outputPaths.codexHooks, "");
  if (codexHooks.trim() && canManageCodexHooks(entrypoints.codexHooks)) {
    upsertCodexHooks(entrypoints.codexHooks);
  }
  ...
  return {
    ...
    adapters: {
      codex: {
        ...
        hooksPath: outputPaths.codexHooks,
        mode: "hooks-plus-project-instructions",
      },
      gemini: {
...
function buildCompiledState(config, mergeIndex, repoProfile, latestEvidence, adapterModes, sharedSpecSources) {
  const payload = {
    ...
    adapterModes,
    sharedSpec: {
      summary: buildSharedSpecSummary(sharedSpecSources),
      sections: Object.fromEntries(
        Object.entries(sharedSpecSources.sections ?? {}).map(([sectionName, meta]) => [
          sectionName,
          meta.acceptedText ?? "",
        ]),
      ),
...
function writeCompiledOutputs(repoRoot, config, mergeIndex, latestEvidence = null, repoProfile = null) {
  ...
  const compiledState = buildCompiledState(
    config,
    mergeIndex,
    nextRepoProfile,
    latest,
  ...
  writeText(
    outputPaths.compiledSpec,
    renderCompiledSpecMarkdown(nextRepoProfile, knowledgeDict, specDict, sharedSpecSources),
  );
  writeText(
    outputPaths.sessionStartInjection,
    renderSessionStartInjection(
      repoRoot,
      nextRepoProfile,
      latest,
      knowledgeDict,
  ...
  const sharedInjection = renderSharedInjection(
    repoRoot,
    nextRepoProfile,
    latest,
    knowledgeDict,
  ...
  writeText(outputPaths.codexHooks, renderCodexHooks());
  writeText(
...
export function ensureCompiledArtifacts(repoRoot) {
  ...
  const compiledState = buildCompiledState(
    ...
    latest,
    adapterModes,
    loadSharedSpecSources(repoRoot),
  );
...
function buildSummaryRequest(inputData, repoRoot, stopConfig, snapshot) {
  const { excerpt: assistantExcerpt, chars: assistantMessageChars } = getAssistantExcerpt(
    inputData,
    stopConfig.maxAssistantExcerptChars,
  );
  const messageFingerprint = fingerprintText(assistantExcerpt).slice(0, 16);
  ...
  return {
    ...
    messageFingerprint,
    assistantMessageExcerpt: assistantExcerpt,
...
function shouldRequestStopSummary(stopConfig, files, lastMessage, transcriptText = "") {
  const normalizedLastMessage = lastMessage.trim();
  const normalizedTranscript = transcriptText.trim();
  const conversationChars =
    normalizedTranscript.length > 0 ? normalizedTranscript.length : normalizedLastMessage.length;
  if (conversationChars < stopConfig.minimumConversationChars) {
    return false;
  }
  if (files.length >= stopConfig.minimumChangedFiles) {
    return true;
  }
  if (!stopConfig.allowWithoutChanges) {
    return false;
  }
  if (normalizedTranscript.length > 0) {
    return true;
  }
  return normalizedLastMessage.length >= stopConfig.minimumAssistantChars;
}
...
export async function mainCodexUserPromptSubmit() {
  const inputData = readStdinJson();
  const repoRoot = resolveHookRepoRoot(inputData);
  ensureCompiledArtifacts(repoRoot);
  const config = loadConfig(repoRoot);
...
export async function mainStop() {
  ...
  if (stopConfig.mode !== "full") {
    approve();
    return 0;
  }
  const captureConfig = getCaptureConfig(config);
  const transcriptText = readTranscriptExcerpt(getTranscriptPath(inputData), captureConfig.maxTranscriptChars);
  ...
  if (getStopHookActive(inputData) || !shouldRequestStopSummary(stopConfig, files, lastMessage, transcriptText)) {
    approve();
    return 0;
  }
...
function runAutoCapture(argv = process.argv.slice(2)) {
  const flags = parseFlagArgs(argv);
  const toolName = String(flags.tool ?? "").trim().toLowerCase();
  if (!["codex", "opencode"].includes(toolName)) {
    return { exitCode: 1, payload: { ok: false, error: `Unsupported Memraft tool: ${String(flags.tool ?? "")}` } };
  }
  const rawCandidates = [];
  try {
  ...
  if (!isCaptureEnabled(repoRoot, toolName)) {
    return { exitCode: 0, payload: { ok: true, skipped: true, reason: "capture_disabled" } };
  }
  const config = loadConfig(repoRoot);
  const captureConfig = getCaptureConfig(config);
  const snapshot = buildCaptureSnapshot(repoRoot, captureConfig);
  ...
  if (skip) {
    return { exitCode: 0, payload: { ok: true, skipped: true, reason } };
  }
...
export async function mainAutoCapture(argv = process.argv.slice(2)) {
  const result = runAutoCapture(argv);
...
export async function mainCodexStop(argv = process.argv.slice(2)) {
  const inputData = readStdinJson();
  const payload = {
    ...inputData,
    cwd: resolveHookRepoRoot(inputData),
  };
  runAutoCapture([
    "--tool",
    "codex",
    "--event-type",
    "Stop",
    ...(getSessionId(inputData) ? ["--session-id", getSessionId(inputData)] : []),
...
function processSessionEndEvent(repoRoot, event) {
  ...
  if (stopConfig.mode === "light") {
    const assistantExcerpt = getStringValue(event, "assistantMessageExcerpt");
    const transcriptText = readTranscriptExcerpt(getStringValue(event, "transcriptPath"), captureConfig.maxTranscriptChars);
    if (!shouldRequestStopSummary(stopConfig, files, assistantExcerpt, transcriptText)) {
      for (const request of unresolvedRequests) {
        request.status = "expired";
        request.updatedAt = String(event.createdAt);
        request.fallbackEventId = String(event.eventId);
    ...
    persistSummary(repoRoot, event, buildLightSessionEndPayload(event, files), "session-end-light", {
      sessionEndLight: true,
      sessionEndEventId: event.eventId,
      stopSummaryMode: stopConfig.mode,
      assistantExcerptChars: String(assistantExcerpt).trim().length,
    });
    for (const request of unresolvedRequests) {
      request.status = "expired";
      request.updatedAt = String(event.createdAt);
...
export async function mainSessionEnd(argv = process.argv.slice(2)) {
  ...
  if (assistantExcerpt) {
    event.assistantMessageExcerpt = assistantExcerpt;
    event.assistantMessageChars = assistantMessageChars;
  }
...
export async function mainMemraftCli(argv = process.argv.slice(2)) {
  ...
  if (args[0] === "accept") {
    const flags = parseFlagArgs(args.slice(2));
    try {
      const payload = acceptSharedProposal(
        repoRoot,
        args[1] ?? "",
        String(flags.into ?? "").trim(),
        nowIso(),
      );
      const config = loadConfig(repoRoot);
      const mergeIndex = loadMergeIndexDb(repoRoot);
      ...
      if (flags.json) {
        ...
        console.log(
          `Accepted ${payload.fingerprint} into ${payload.acceptedInto} (${payload.filePath})`,
        );
      }
      return 0;
    } catch (error) {
  ...
  if (args[0] === "inspect") {
    ...
    if (subcommand === "proposals") {
      const payload = listSharedSpecProposals(repoRoot);
      if (flags.json) {
        printJson(payload);
      } else if (payload.length === 0) {
        console.log("Shared Spec Proposals\n\n- none");
      } else {
        console.log("Shared Spec Proposals\n");
        for (const item of payload) {
          console.log(`- ${item.fingerprint} (${item.collection} -> ${item.recommendedSection})`);
          console.log(`  ${getStringValue(item.record, "text")}`);
        }
      }
      return 0;
    }
